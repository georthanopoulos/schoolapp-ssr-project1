...
@Configuration                            // That's why methods can be characterized as 'Bean'. It enables the @Bean feature.
@EnableWebSecurity                        // Filter Security
@EnableMethodSecurity                     // Enables the @PreAuthorize
@RequiredArgsConstructor                  // Dependency Injection (DI)
public class SecurityConfig {

    private final AuthenticationSuccessHandler authSuccessHandler;             // "Horizontal" auditing for all the controllers of the App, are being performed.
    private final CustomAuthenticationFailureHandler authFailureHandler;

    @Bean
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http
//                .csrf(AbstractHttpConfigurer::disable)                      // Filter -- Cross-Site Request Forgery  --the default value is true! MUST BE ALWAYS ENABLED (=true) at Production Level!!!
                ...
                .logout(logout -> logout
                        .logoutSuccessUrl("/login?logout=")         // Works with Post
